...
func handleRequests(requests []Request, etcdClient *clientv3.Client) map[string]interface{} {
    ...
    for _, req := range requests {
        ...
        // Read request
        if req.Op == "read" {
            if val, ok := cache.Get(req.Key); ok {
                cliResp[req.RID] = val
            } else {
                if _, ok := dedupReqs[req.Key]; !ok {
